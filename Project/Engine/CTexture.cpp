...
{
	m_Desc.Width = _Width;          // DepthStencil Texture는 Render Target 해상도와 반드시 일치해야 함
	m_Desc.Height = _Height;
	m_Desc.Format = _PixelFormat;   // 텍스쳐의 픽셀 형식 (ex. Depth 24bit, Stencil 8bit)
	m_Desc.ArraySize = 1;
	m_Desc.BindFlags = _Flags;

	m_Desc.Usage = _Usage;          // System Memory와의 연계 설정
	...
	m_Desc.MiscFlags = 0;
	m_Desc.MipLevels = 1;           // 열화버전 해상도 이미지 추가 생성 기능 (1은 추가 X)

	m_Desc.SampleDesc.Count = 1;    // 멀티 샘플링의 샘플 수 (1은 비활성화)
	m_Desc.SampleDesc.Quality = 0;  // 멀티 샘플링의 품질 수준
...
// 기존에 존재하던 Texture2D를 CTexture 객체로 생성하는 Create 함수
